...
int get_codepage(ExtensionAPI::Pane p) {
	...
	if (codePage != SA::CpUtf8/*SC_CP_UTF8*/) {
		SA::CharacterSet cs = SA::CharacterSet::Default /*SC_CHARSET_DEFAULT*/;
		std::string charSet = host->Property("character.set");
		if (!charSet.empty())
			cs = static_cast<SA::CharacterSet>(atoi(charSet.c_str()));
...
#ifdef RB_GETCURWORD
static int cf_get_current_word(lua_State* L) {
	auto wEditor = host->PaneCaller(host->paneEditor);
...
//!-start-[OnSendEditor]
const char* CallNamedFunction(const char* name, unsigned int numberArg, unsigned int numberArg2, const char* stringArg) {
	const char* handled = NULL;
	if (luaState) {
		lua_getglobal(luaState, name);
		if (lua_isfunction(luaState, -1)) {
...
const char* CallNamedFunction(const char* name, unsigned int numberArg, const char* stringArg, int numberArg2) {
	const char* handled = NULL;
	if (luaState) {
		lua_getglobal(luaState, name);
		if (lua_isfunction(luaState, -1)) {
...
const char* CallNamedFunction(const char* name, unsigned int numberArg, unsigned int numberArg2, long numberArg3) {
	const char* handled = NULL;
	if (luaState) {
		lua_getglobal(luaState, name);
		if (lua_isfunction(luaState, -1)) {
...
const char* LuaExtension::OnSendEditor(Scintilla::Message msg, uintptr_t wp, const char* lp) {
	return CallNamedFunction("OnSendEditor", (unsigned int)msg, wp, lp);
}
