...
void SciTEBase::SetOneStyle(GUI::ScintillaWindow &win, int style, const StyleDefinition &sd) {
	...
	if (sd.specified & StyleDefinition::sdHotspot)
		win.StyleSetHotSpot(style, sd.hotspot ? 1 : 0);//win.Send(SCI_STYLESETHOTSPOT, style, sd.hotspot ? 1 : 0);
	//!-end-[StyleDefHotspot]
#endif // RB_HOTSPOT
...
void SciTEBase::ReadProperties() {
...
#ifdef RB_CARET
	//!-start-[caret]
	std::string tmp_str = props.GetNewExpandString("caret.fore.", fileNameForExtension.c_str());
	...
	if (tmp_str.length()) {
		wEditor.SetCaretWidth(IntFromHexByte(tmp_str.c_str()));
		wOutput.SetCaretWidth(IntFromHexByte(tmp_str.c_str()));
	}
	else {
#endif // RB_CARET
	...
	if (props.GetInt("margin.bookmark.by.single.click", 1) == 1) //!-add-[SetBookmark]
		wEditor.SetMarginSensitiveN(1, true);//wEditor.Call(SCI_SETMARGINSENSITIVEN, 1, 1);	//!-add-[SetBookmark]
#endif
...
void SciTEBase::ReadFontProperties() {
	...
	//!-start-[StyleDefault]
#if !defined(GTK)
	StyleDefinition style(sval);
	char sColor[8];
	DWORD color;/*ColourDesired*/
	if (!(style.specified & StyleDefinition::sdBack)) {
		color = ::GetSysColor(COLOR_WINDOW);  //color.Set(::GetSysColor(COLOR_WINDOW));
		memset(sColor, 0, sizeof(sColor));
		sprintf(sColor, "#%2X%2X%2X", color & 0xFF /*GetRed*/, (color>>8) & 0xFF/*GetGreen*/, (color>>16) & 0xFF /*GetBlue*/);
		style.back = sColor;
	...
	if (!(style.specified & StyleDefinition::sdFore)) {
		color = ::GetSysColor(COLOR_WINDOWTEXT); //color.Set(::GetSysColor(COLOR_WINDOWTEXT));
		memset(sColor, 0, sizeof(sColor));
		sprintf(sColor, "#%2X%2X%2X", color & 0xFF /*GetRed*/, (color>>8) & 0xFF/*GetGreen*/, (color>>16) & 0xFF /*GetBlue*/);
		style.fore = sColor;
	...
	SetOneStyle(wEditor, StyleDefault, style);
	SetOneStyle(wOutput, StyleDefault, style);
#else
	SetOneStyle(wEditor, StyleDefault, StyleDefinition(sval));
	SetOneStyle(wOutput, StyleDefault, StyleDefinition(sval));
#endif
