...
GUI::ScintillaWindow &SciTEBase::PaneFocused() {
	return wOutput.HasFocus() ? wOutput : reinterpret_cast<GUI::ScintillaWindow&>(wEditor) /*wEditor*/;
}
...
#ifdef RB_ACI
	if (!current) wEditor.AutoCCancel(); //!-add-[autocompleteword.incremental]
#endif // RB_ACI
...
#ifdef RB_ENCODING
	const std::string expbuf = EncodeString( UnSlashString(data.c_str()) );//>!-add-[FixEncoding]
#else
...
void SciTEBase::MenuCommand(int cmdID, int source) {
	...
	case IDM_ENCODING_UCOOKIE:
		...
		if (wEditor.CodePage()/*Call(SCI_GETCODEPAGE)*/ != codePage) {
			codePage = 0;
			wEditor.SetCodePage(codePage);//Call(SCI_SETCODEPAGE, codePage);
		}
	...
	case IDM_READONLY:
		...
		SetBuffersMenu();
#ifdef RB_ROTM
		//BuffersMenu(); //!-add-[ReadOnlyTabMarker] disabled
#endif // RB_ROTM
...
void SciTEBase::Notify(SCNotification *notification) {
	...
	case SA::Notification::DoubleClick:
...
#ifdef RB_GMI
			handled = GoMessage(0); //!-add-[GoMessageImprovement]
#else
			GoMessage(0);
#endif // RB_GMI
		...
		if (handled) {
			if (notification->nmhdr.idFrom == IDM_RUNWIN)
				wOutput.Call(SA::Message::Click /*SCI_SETMOUSECAPTURE*/, 0);
			else
				wEditor.Call(SA::Message::Click /*SCI_SETMOUSECAPTURE*/, 0);
		}
		break;
	...
	case SA::Notification::HotSpotReleaseClick /*SCN_HOTSPOTRELEASECLICK*/:
		...
		//!-start-[GoMessageImprovement]
	case SA::Notification::MouseButtonUp/*SCN_MOUSEBUTTONUP*/:
		if (extender)
	...
	case SA::Notification::MarginClick: {
			...
			if (!handled) {
				...
				if (notification->margin == 1) {
					auto lineClick = wEditor.LineFromPosition(notification->position);//int(wEditor.Call(SCI_LINEFROMPOSITION, notification->position));
...
bool SciTEBase::IsMenuItemEnabled(int cmd) {
	...
	case IDM_SAVE:
		...
		break;
	case IDM_UNDO:
		return CallFocused(SA::Message::CanUndo/*SCI_CANUNDO*/);
		break;
	case IDM_REDO:
		return CallFocused(SA::Message::CanRedo/*SCI_CANREDO*/);
		break;
	case IDM_DUPLICATE:
	...
	case IDM_COPY:
		...
		//break;
	case IDM_CLEAR:
		return CallFocused(SA::Message::GetSelectionStart/*SCI_GETSELECTIONSTART*/) != CallFocused(SA::Message::GetSelectionEnd/*SCI_GETSELECTIONEND*/);
		break;
	case IDM_PASTE:
		return CallFocused(SA::Message::CanPaste/*SCI_CANPASTE*/);
		break;
