...
// открывает файл, вместо €рлыка файла
#define RB_LINK

// [user_shortcuts]
#define RB_USERSC

// добавл€ет экспорт событи€ перемещени€ вкладок
#define RB_ONTABMOVE
...
// альтернативна€ иконка главного окна просле 354-го дн€ года (SciTExmas.ico)
#define RB_DOY

// [clear_before_execute]
// оверрайд clearbefore:[1,0,yes,no] глобальной настройки clear.before.execute дл€ конкретной команды
#define RB_CBE

// props['GetCurrentWord']
#define RB_GETCURWORD

// скрывает список автозавершени€ при перемещении окна
#define RB_ACCANCELONMOVE

// дл€ пользовательских гор€чих клавиш позвол€ет задавать 'Num *' вместо 'KeypadMultiply' и т.д.
#define RB_USCNUM

// скрывает панель фильтра при переключении вкладок props["hide.filterstrip.on.switch.tab"]
#define RB_HFS

// автопрокрутка экрана с сохранением положени€ активной строки
#define RB_MCIVEX
//-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
