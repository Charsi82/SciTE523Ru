...
protected:
	...
	//!-start-[user.toolbar]
	void SetToolBar();
	std::map<uptr_t, std::string> ToolBarTips;
	HBITMAP hToolbarBitmap;
	UINT_PTR oldToolbarBitmapID;
	std::vector<int> toolbarUsersPressableButtons;
	//!-end-[user.toolbar]
#endif // RB_UT
...
#ifdef RB_SUBMENU
	MenuEx GetToolsMenu() override { return MenuEx(((GUI::MenuID)::GetSubMenu(::GetMenu(reinterpret_cast<HWND>(wSciTE.GetID())), menuTools))); }; //!-add-[SubMenu]
#endif // RB_SUBMENU
