...
long SciTEKeys::ParseKeyCode(const char *mnemonic) {
	...
	if (mnemonic && *mnemonic) {
		...
		if (sKey.length() == 1) {
			...
			keyval = VkKeyScan(sKey.at(0));
			if (keyval == -1)
			{
				if (sKey.at(0) >= 'A' && sKey.at(0) <= 'Z')
					keyval = sKey.at(0);
			}
			else
				keyval &= 0xFF;
...
#ifdef RB_USCNUM
			} else if (StartsWith(sKey, "Num ")) {
				sKey.erase(0, strlen("Num "));
				if ((sKey.length() > 0) && IsADigit(sKey.at(0))) {
					const int keyNum = atoi(sKey.c_str());
					if (keyNum >= 0 && keyNum <= 9)
						keyval = keyNum + VK_NUMPAD0;
...
#ifdef RB_USERSC
	// user shortcuts
	HMENU mainMenu = GetMenu(MainHWND());
	for (const auto& p : shortCutItemList)
	{
		int cmdID = GetMenuCommandAsInt(p.menuCommand);
		GUI::gui_char buff[256]{};
		MENUITEMINFO mii{};
		...
		if (GetMenuItemInfo(mainMenu, cmdID, FALSE, &mii)) {
			...
			if (pos != std::string::npos)
				tmp.insert(pos + 1, p.menuKey).insert(pos + 1 + p.menuKey.size(), ", ");
			else
				tmp.append("\t").append(p.menuKey);
			lstrcpy(buff, GUI::StringFromUTF8(tmp).data());
			SetMenuItemInfo(mainMenu, cmdID, FALSE, &mii);
		}
	}
#endif
}
...
#ifdef RB_TABTOP
	toptab_h = std::clamp(props.GetInt("tabbar.top.height", 2), 0, 5);
			...
			if ((bytesAvail < 1000) && (hWriteSubProcess != INVALID_HANDLE_VALUE) && (writingPosition < totalBytesToWrite)) {
				...
				if (bTest && bytesRead) {
					...
					if (!(jobToRun.flags & jobQuiet)) {
						...
						if (props.GetInt("output.code.page.oem2ansi") && (wEditor.LexerLanguage() != "python")) {
	...
	if (extender) {
		...
		if (bIsDeadKey) {
			...
			if (sPrevIsDeadKey == false) {
				...
				if (::ToAscii((UINT)wParam, MapVirtualKey((UINT)wParam, 0), masKS, (LPWORD)ch, uFlags) != 1) {
					ch[0] = 0;
				}
			}
...
#ifdef RB_ACCANCELONMOVE
			wEditor.AutoCCancel();
#endif // RB_ACCANCELONMOVE
			break;
