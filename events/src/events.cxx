...
void dump_stack(lua_State* L)
{
	...
	for (int i = 1; i <= sz; i++)
	{
		...
		if (str) {
			...
			lua_print(L, lua_typename(L, lua_type(L, i)));
		}
		lua_pop(L, 1);
	}
	lua_pop(L, 1);
	lua_print(L, "===== dump end ====");
}
...
public:
	std::string name;
	Event(const char* _name) { in_progress = 0; name = _name; finger_print = ""; m_break = m_removeThisCallback = false; };
	Event(const Event& e) { name = e.name; m_break = m_removeThisCallback = false; };
	~Event() = default;
	// methods
...
public:
	...
	CEvtManager() = default;
	~CEvtManager() {
		for (auto& it : map_events) delete it.second;
		map_events.clear();
	};
};
...
{
	Event* ev_object = (Event*)lua_touserdata(L, idx);
