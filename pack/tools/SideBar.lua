...
local function ReplaceWithoutCase(text, s_find, s_rep)
	s_find = string.gsub(s_find, '.', function(ch)
		if ch:match('%d') then return ch end -- avoid %digit subpattern
...
tabs:add_tab("Проводник", tab0)
tabs:add_tab("Функции", tab1)
tabs:add_tab("Сокращения", tab2)
...
AddEventHandler("OnTabMove", function(from, to)
	for _, bmk in ipairs(table_bookmarks) do
		if bmk.BufferNumber == from then bmk.BufferNumber = to 
		elseif  bmk.BufferNumber == to then bmk.BufferNumber = from end
	end
	Bookmarks_ListFILL()
end)
...
list_dir:context_menu {
	...
	'Показать c расширением выделенного файла|FileMan_MaskOnlyCurrentExt',
	'', -- separator
