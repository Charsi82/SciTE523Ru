...
{
	send_msg(BM_SETCHECK, c ? TRUE : FALSE);
}
...
{
	send_msg(BM_SETSTATE, s ? TRUE : FALSE);
}
...
TCheckBox::TCheckBox(TWin* parent, pchar caption, int id, bool is_auto)
	: TButton(parent, caption, id, is_auto ? TButton::AUTOCHECKBOX : TButton::CHECKBOX)
{
}
...
{
	send_msg(LB_ADDSTRING, 0, (LPARAM)str);
	if (ptr) {
...
{
	send_msg(LB_SETITEMDATA, (WPARAM)i, (LPARAM)ptr);
}

void* TListBox::get_data(int i)
{
	return (void*)send_msg(LB_GETITEMDATA, (WPARAM)i);
}
...
TTrackBar::TTrackBar(TWin* parent, int id)
	: TControl(parent, TRACKBAR_CLASS, L"", id, TBS_AUTOTICKS), m_redraw(false)
{}
...
/*WNDFN*/ LRESULT FormWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam);

void subclass_control(TControl* ctrl)
{
	HWND hwnd = (HWND)ctrl->handle();
	ctrl->m_wnd_proc = (void*)GetWindowLongPtr(hwnd, GWLP_WNDPROC);
	SetWindowLongPtr(hwnd, GWLP_WNDPROC, (LONG_PTR)FormWndProc);
}

void remove_subclass_control(TControl* ctrl)
{
	if (ctrl->m_wnd_proc)
		SetWindowLongPtr((HWND)ctrl->handle(), GWLP_WNDPROC, (LONG_PTR)ctrl->m_wnd_proc);
}
...
/*WNDFN*/LRESULT FormWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	TEventWindow* ew = (TEventWindow*)GetWindowLongPtr(hDlg, 0);
	switch (msg) {
	case WM_KEYDOWN:
		if (wParam == VK_TAB) {
