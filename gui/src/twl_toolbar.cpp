...
void TToolbar::create()
{
	...
	send_msg(TB_SETBITMAPSIZE, 0, (LPARAM)MAKELONG(m_bwidth, m_bheight));
...
void TToolbar::add_item(pchar bmp, pchar tooltext, EventHandler eh, void* data)
{
	...
	btn.iString = 0;
	send_msg(TB_ADDBUTTONS, 1, (LPARAM)&btn);
}
...
public:
	...
	int handle_notify(void* p)
	{
		...
		if (lpToolTipText->hdr.code == (UINT)TTN_GETDISPINFO) {
			TBBUTTON btn{};
			UINT_PTR id = lpToolTipText->hdr.idFrom;                   // command id
			auto index = m_tb->send_msg(TB_COMMANDTOINDEX, id);    // index in the buttons
			...
			if (btn.dwData == 0)
				return 0;
			wcscpy_s(lpToolTipText->szText, (pchar)btn.dwData);
		}
		return 1;
	}
};
...
TToolbar& operator<< (TToolbar& tb, Item item)
{
	...
	wchar_t* bmp_file = wcstok_s(caption, L":", &next_token);
	wchar_t* tooltip_text = wcstok_s(NULL, L"", &next_token);
...
int TTreeView::add_item(pchar text, int idx, void* data)
{
	this->add(NULL, text);
	return 0;
}

Handle TTreeView::add(Handle parent, pchar caption, int idx1, int idx2, bool has_children, void* data)
{
	TVITEM item{};
...
int TTreeView::handle_notify(void* p)
{
	...
	case TVN_SELCHANGED:
		if (m_on_select)
			(m_form->*m_on_select)((intptr_t)item);
		return 1;
	case TVN_SELCHANGING:
		if (m_on_selection_changing)
			return (m_form->*m_on_selection_changing)((intptr_t)item);
		else return 0;
	}
	return 0;
}
