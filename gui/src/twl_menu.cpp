...
void Menu::add_menu(Menu& menu)
{
	...
	AppendMenu(HMENU(m_handle), MF_STRING | MF_POPUP, (UINT_PTR)menu.m_handle, name);
...
void MessageHandler::remove(UINT id)
{
	...
	//m_list->remove_if([id](Item item) {item.id == id;});
}
...
size_t list_size(ItemList* list)
{
	return list->size();
}
