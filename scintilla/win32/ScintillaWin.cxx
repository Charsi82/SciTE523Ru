...
#ifdef RB_IMLTPT
	void InsertMultiPasteText(const char* text, size_t len); //!-add-[InsertMultiPasteText]
#endif // RB_IMLTPT
...
void ScintillaWin::NotifyDoubleClick(Point pt, KeyMod modifiers) {
	...
	::SendMessage(MainHWND(),
			  WM_LBUTTONDBLCLK,
#ifdef RB_ODBCLK // note: alt pressed will be added in ScintillaWin::MouseModifiers(...)
		(FlagSet(modifiers, KeyMod::Ctrl) ? MK_CONTROL : 0) | // add [OnDoubleClick]
#endif // DEBUG
...
void ScintillaWin::InsertMultiPasteText(const char* text, size_t len) {
	...
	for (size_t r = 0; r < sel.Count(); r++) {
		if (!RangeContainsProtected(sel.Range(r).Start().Position(),
			...
			positionInsert = RealizeVirtualSpace/*InsertSpace*/(positionInsert, sel.Range(r).caret.VirtualSpace());
...
void ScintillaWin::Paste() {
	...
	if (const wchar_t *uptr = static_cast<const wchar_t *>(memUSelection.ptr)) {
		...
		//!-start-[InsertMultiPasteText]
		if (!isMultiPaste) {
			InsertPasteShape(&putf[0], putf.length(), pasteShape);
		}
		else {
			InsertMultiPasteText(&putf[0], putf.length());
		}
		//!-end-[InsertMultiPasteText]
#else
